...
- `Deployment/grok2api`：官方 v3.1.2 Grok 网关，`Service/grok2api` 当前承载该版本。
...
- `kiro-rs` 当前固定使用 ZyphrZero 官方二次开发版 `docker.io/zyphrzero/kiro-rs:0.7.5@sha256:78c4dbea0cfba2614c3c71747a939324ffba07535c703ecfb18c150d3c719de9`，通过 initContainer 将 `config.json` 与初始 `credentials.json` 复制到可写 PVC，避免 Token 刷新后无法回写；旧版的 config 与 credentials 可直接沿用，当前代理指向 `http://mihomo-gpt-listener.default.svc.cluster.local:7910`。
- `cli-proxy-api` 通过 initContainer 将 `config.yaml` 复制到 PVC，并初始化持久化 `auths` 目录；当前固定使用官方 `eceasy/cli-proxy-api:v7.2.135@sha256:53162ac4ebf4f399b729a80830ad13e992add2430e1387a54e09a192987f6df3`（2026-08-17 发布并核验；`linux/amd64` manifest `sha256:842d0eb49caa5bc704cbd29cfc804406e41d9985a953429991ceff1a0878d790`）。7.2.134–7.2.135 改进流错误保留、`service_tier` 映射、鉴权会话与 cooling 管理，并更新 GPT-5.6 上下文限制；`/CLIProxyAPI/config.yaml`、`/root/.cli-proxy-api`、端口 `8317` 与现有 PVC 保持兼容，无数据库或路径迁移。
- `grok2api` 当前固定使用官方 `ghcr.io/chenyme/grok2api:v3.1.2@sha256:6b4067c5d9e84b13b2fc4f8ec3b3d91ee9b560a80cbbf4e8c3e40b5f8d19e3f3`。配置从 `grok2api-secret` 挂载到 `/run/grok2api/config.yaml`，数据位于原名 PVC 的 `/app/data`；无官方迁移要求，但 `ModelSpec SearchTools` 已移除，继续固定 `v3.1.2`，不使用 `latest`。
...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.8.2@sha256:19160cad1646bf1e141aa88cc3054349aa248b8bc4162eb22b789c23012ef88b`（2026-08-18 发布并核验；`linux/amd64` manifest `sha256:651dbe5ef76fa33bf8024c41d0cf125a72573f61dbcfa1a688aeb050320056a7`；官方 release：https://github.com/james-6-23/codex2api/releases/tag/v2.8.2），按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`。2.8.1–2.8.2 改进 Claude Code/Grok cache/Responses、unknown provider 换号与流式 Remote Compact 修复，可选 Grok follow-up effort 默认关闭；无数据库迁移或破坏性 `/v1/*` 变化，端口 `8080`、`/health`、Redis 配置与 `/data` 保持不变。
