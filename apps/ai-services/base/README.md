...
- `cli-proxy-api` 通过 initContainer 将 `config.yaml` 复制到 PVC，并初始化持久化 `auths` 目录；当前固定使用 `eceasy/cli-proxy-api:v7.2.48@sha256:8361d362e02394d4f28b290b0bbc7f426bb8c579ebd8064435f13605b46eb5a5`。
- `grok2api` 当前固定使用 `ghcr.io/xianyudaxian/grok2api:latest@sha256:2557b20ec1b1979e8e3afea5d7936e5b1100e028e3655c0b37e1fae48cd779bb`（对应 `https://github.com/XianYuDaXian/grok2api` fork 的 GHCR 发布镜像），通过 initContainer 首次将 Secret 中的 `config.toml` 复制到 PVC；后续运行时配置、SQLite 账号库与日志都保存在同一个持久化卷中。
...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.3.9@sha256:518df6ad5cda1d6c1c9013e87157fcc5e9b5166318c6624509cada8827340429`，按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`。
