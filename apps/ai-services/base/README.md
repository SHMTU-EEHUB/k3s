...
- `cli-proxy-api` 通过 initContainer 将 `config.yaml` 复制到 PVC，并初始化持久化 `auths` 目录；当前固定使用 `eceasy/cli-proxy-api:v7.2.128@sha256:3f7a734784f4cc2c86f6268929caff1b0c178fe600fdaa12cefe884dc4cae841`，配置保持不变。
...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.7.4@sha256:6b97b20aa408f9e74fa1988b89c1028ad0efec590ca111c86f42d413bbe72764`，按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`；`2.7.3`→`2.7.4` 无数据库迁移，可直接替换镜像；默认重试、CY 锁 TTL、Key 隔离、HTTP/2 轮转和断流阈值均有变化。
