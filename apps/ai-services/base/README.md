...
- `cli-proxy-api` 通过 initContainer 将 `config.yaml` 复制到 PVC，并初始化持久化 `auths` 目录；当前固定使用 `eceasy/cli-proxy-api:v7.2.130@sha256:7fca20b67690f919d88a868e3dcfcba8bbda5f42fef9e5169c0b95853ff6fb8c`，配置、端口、`auths` 目录与 PVC 保持不变。该版本修复 OpenAI SSE 提前终止、Codex WebSocket、多代理 namespace、session header 和 `prompt_cache_options`。
...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.7.6@sha256:10c947438862c9e48d459be59ebe00c07e28a7423c38a524b02e33cc8e4fddc3`，按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`；无必需配置、端口或 PVC 变化。新增官方结算用量持久化、CY 用户冷却与设备指纹策略，并修复性能、UI 和统计；已有数据库会自动建表/索引，未发现人工迁移要求。
