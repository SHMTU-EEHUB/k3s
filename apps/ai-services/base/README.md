...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.8.1@sha256:2875ed7cfbf7a416fd1401a87c080d63b08004f11cde6b380dabac1a2697548c`（2026-08-17 发布并核验；`linux/amd64` manifest `sha256:fd6be1cedb4b9a0d1621ca46745d951627c1af935d9a9438f55e6d85fbc779d8`），按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`。2.8.0–2.8.1 改进大账号池统计与清理、OAuth 导入重试、失败流识别、提示词风险索引与审核配置，并新增默认关闭的 ChatGPT Live 接口；数据库索引和 `prompt_review_profiles` 由应用自动创建，无需人工迁移，端口 `8080`、`/health`、Redis 配置与 `/data` 保持不变。
