...
- `Deployment/grok2api`：官方 v3.1.1 Grok 网关，`Service/grok2api` 当前承载该版本。
...
- `cli-proxy-api` 通过 initContainer 将 `config.yaml` 复制到 PVC，并初始化持久化 `auths` 目录；当前固定使用 `eceasy/cli-proxy-api:v7.2.120@sha256:88b1770274ba45a13dd6f2d746b3cd0c1120a8ddbd371da94251bc33a54a8a3b`。
- `grok2api` 当前固定使用官方 `ghcr.io/chenyme/grok2api:v3.1.1@sha256:080fcd8003ab5991346441f450e67bd13e488b28d17e5f561384b49bbc9b6c63`。配置从 `grok2api-secret` 挂载到 `/run/grok2api/config.yaml`，数据位于原名 PVC 的 `/app/data`；当前已使用 v3 数据格式，v3 不迁移或复用旧 v2 数据格式。
...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.7.1@sha256:cc16f8af03e96d146b09950e0fb3e43e0afbc66dcb7c329cbbbec02e95edc582`，按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`。
