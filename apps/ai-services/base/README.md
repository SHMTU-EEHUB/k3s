...
- `cli-proxy-api` 通过 initContainer 将 `config.yaml` 复制到 PVC，并初始化持久化 `auths` 目录；当前固定使用 `eceasy/cli-proxy-api:v7.2.133@sha256:944f8b1f5d23981244b5d482290d411d2623bf685749a74c5bca849c865c0ac8`。7.2.131–7.2.133 新增请求重试覆盖、Grok Imagine 与模型元数据，修复响应、工具与 WebSocket，并更新 `max_completion_tokens` 和 GPT-5.6 Sol 模型注册；配置向后兼容，无数据库、端口或路径迁移。
...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.7.9@sha256:f264eec71dec54cd527781affce2fb6d5fed03a04f9915615a36ea6234c73c4f`，按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`。2.7.7–2.7.9 新增 Grok 控制/媒体、提示词锁、图片尺寸、Responses/WebSocket/账号对账、过载暂停和导入流式；数据库字段与索引自动增补，无需人工迁移，端口、`/health` 与 `/data` 保持不变。
...
- `gemini-web2api` 当前固定使用 `ghcr.io/sophomoresty/gemini-web2api:latest@sha256:09c00179de3c1cb0d084e1c38f3bf77177fd6795a951a18397c29d7e1263dcb9`（2026-08-11 构建），通过 `ConfigMap/gemini-web2api-config` 挂载 `/app/config.json`，监听 `0.0.0.0:8081`；配置内设置 `temporary_chats: false` 并保留 `api_keys: []`，不启用应用层 API key。`gemini_bl` 会在启动时自动刷新，当前配置值仅作为 fallback；服务使用 Mihomo 默认代理 `http://mihomo-proxy-nodeport.default.svc.cluster.local:7897`。
