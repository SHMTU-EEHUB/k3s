...
- `cli-proxy-api` 通过 initContainer 将 `config.yaml` 复制到 PVC，并初始化持久化 `auths` 目录；当前固定使用 `eceasy/cli-proxy-api:v7.2.15@sha256:4af868d162d2c42c09eee817a7c9dc5d0726ee0bca86b65ac1b7032afb22b13c`。
...
- `codex2api` 使用 initContainer 幂等创建 `codex2api` 数据库与用户；当前固定使用 `ghcr.io/james-6-23/codex2api:2.3.5@sha256:f5f843cef204963ebdd7043b764822c1d51588b21fbe699f6c636d4774d43f90`，按 PostgreSQL + Redis 缓存运行，使用 `ai-services-redis` 的 database 1，运行期图片与日志保存在 `/data`。
...
- WA App（`wa-app`）当前固定使用 `ghcr.io/pood1e/wa-app-service:latest@sha256:451d32317c4706a3af7c86803d153f04226d7daa47af1f36d0c63f5366e2eb49`，gRPC 监听 `50091`，Dashboard 监听 `8080`，数据目录固定为 `/var/lib/wa-app`，健康检查使用 `/healthz`。当前不接共享 PostgreSQL / Redis，按上游默认 SQLite 运行，并通过 `WA_COMMON_PROXY` 和标准代理环境变量走 GPT Mihomo 节点。
